...
- **Model Availability**: To verify which Flash models are available in the `global` location, run:
  ...
  ```
- **Location vs. Region**: Distinguish between the **deployment region** (e.g., `europe-west1`) and the **code location** (always use `global` for Gemini 2.5+ models in this project).
- **Remote Query API**: Deployed `AgentEngine` objects use `async_stream_query(...)`. Do **not** use `async_query(...)` as it will raise an `AttributeError`.
